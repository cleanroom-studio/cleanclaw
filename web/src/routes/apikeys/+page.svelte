<script lang="ts">
  ...
  let name = $state("");
  let type = $state("user");
  let newKey = $state<{ name: string; token: string; type: string } | null>(
    null,
  );
  ...
  async function rotate(id: string) {
    if (!confirm("Rotate this key? The old token stops working immediately."))
      return;
...
<div class="p-6 max-w-4xl mx-auto space-y-4">
  <div>
    ...
    <p class="text-sm text-zinc-400 mt-1">
      Issue, rotate, and revoke API keys for programmatic access.
    </p>
  </div>
  ...
  {#if newKey}
    <Card>
      ...
      <pre
        class="bg-zinc-900 p-3 rounded text-xs overflow-x-auto font-mono">{newKey.token}</pre>
      <button
        class="text-xs text-violet-300 mt-2"
        onclick={() => (newKey = null)}>Dismiss</button
      >
    </Card>
  {/if}
  ...
  {#if rotatedToken}
    <Card>
      ...
      <pre
        class="bg-zinc-900 p-3 rounded text-xs overflow-x-auto font-mono">{rotatedToken}</pre>
      <button
        class="text-xs text-violet-300 mt-2"
        onclick={() => (rotatedToken = null)}>Dismiss</button
      >
    </Card>
  {/if}
            ...
            <tr class="border-b border-zinc-800/50">
              ...
              <td class="py-2 text-right space-x-2">
                <button
                  ...
                  onclick={() => rotate(k.id)}>Rotate</button
                >
                <button
                  class="text-xs text-red-400 hover:underline"
                  onclick={() => remove(k.id)}>Delete</button
                >
              </td>
            </tr>
          {/each}
