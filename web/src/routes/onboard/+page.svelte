<script lang="ts">
  ...
  async function createAdmin(e: Event) {
    ...
    try {
      ...
      if (!r.ok) { error = r.error || 'failed'; creating = false; return; }
      me = { id: r.user_id!, username, email, display_name: displayName, role: 'super_admin', is_admin: true };
      step = 2;
    } catch (err) {
  ...
  function next() { step = Math.min(5, step + 1); }
  function prev() { step = Math.max(0, step - 1); }
</script>
  ...
  <div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-2 text-xs text-zinc-500">
      {#each ['Welcome', 'Admin', 'Provider', 'Agent', 'Sandbox', 'Launch'] as label, i (label)}
        <div class:font-semibold={step === i} class:text-violet-300={step === i}>
          {i + 1}. {label}
        </div>
    ...
    {:else if step === 1}
      <Card>
        ...
        <form onsubmit={createAdmin} class="space-y-3">
          <div><Label for="ad-u">Username</Label><Input id="ad-u" bind:value={username} required /></div>
          <div><Label for="ad-dn">Display name</Label><Input id="ad-dn" bind:value={displayName} placeholder="Admin" /></div>
          <div><Label for="ad-e">Email</Label><Input id="ad-e" type="email" bind:value={email} required /></div>
          <div><Label for="ad-p">Password (min 8)</Label><Input id="ad-p" type="password" bind:value={password} minlength={8} required /></div>
    ...
    {:else if step === 2}
      <Card>
        ...
        <p class="text-sm text-zinc-400">Providers are LLM backends (OpenAI, Anthropic, minimax, Ollama, …). You'll need at least one to drive agent turns.</p>
    ...
    {:else if step === 3}
      <Card>
        ...
        <p class="text-sm text-zinc-400">Agents are the LLM-backed personas that respond to messages. Each agent has its own workspace, skills, channels, and chat history.</p>
        <div class="mt-4 flex justify-between">
          <Button variant="outline" onclick={prev}>← Back</Button>
          <div class="flex gap-2">
            <Button variant="outline" onclick={() => (step = 5)}>Skip</Button>
    ...
    {:else if step === 4}
      <Card>
        ...
        <p class="text-sm text-zinc-400">The sandbox runs tool calls in isolation. Default: <code class="bg-zinc-800 px-1 rounded">docker</code>. Operators can swap to E2B or BoxLite at runtime.</p>
        <div class="mt-4 flex justify-between">
          <Button variant="outline" onclick={prev}>← Back</Button>
    ...
    {:else if step === 5}
      <Card>
        ...
        <p class="text-sm text-zinc-400">
          CleanClaw is ready. Head to the dashboard to start chatting, or jump to a specific section:
        </p>
        <div class="mt-4 grid grid-cols-2 gap-2">
          <a href="/overview/"><Button class="w-full">Overview</Button></a>
          <a href="/chat/"><Button class="w-full" variant="outline">Chat</Button></a>
          <a href="/agents/"><Button class="w-full" variant="outline">Agents</Button></a>
          <a href="/providers/"><Button class="w-full" variant="outline">Providers</Button></a>
        </div>
      </Card>
    {/if}
  </div>
