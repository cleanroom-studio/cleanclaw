...
<div class="p-6 max-w-2xl mx-auto space-y-4">
  ...
  <p class="text-sm text-zinc-400">Deployment-wide knobs. Changes take effect on the next gateway restart.</p>
  <Card>
    <div class="space-y-3">
      <div>
        <Label for="rt-sb">Sandbox backend</Label>
        <Input id="rt-sb" bind:value={sandboxBackend} placeholder="docker / e2b / boxlite" />
      </div>
      <div>
      ...
      <div>
        ...
        <Input id="rt-ll" bind:value={logLevel} placeholder="info / debug / warn" />
      </div>
      <p class="text-xs text-zinc-500">Saving is a no-op in this build — the values are read by the gateway on boot.</p>
    </div>
  </Card>
</div>
