<script lang="ts">
  ...
  let globalTypes = $state<Array<{ type: string; label: string; configured: boolean; logo: string }>>([]);
  let myChannels = $state<Array<{ type: string; account_id: string; enabled: boolean }>>([]);
  let error = $state('');
...
<div class="p-6 max-w-4xl mx-auto space-y-4">
  <div>
    ...
    <p class="text-sm text-zinc-400 mt-1">
      Each channel is a real-time IM (Telegram, Discord, Slack, …) that an agent can use to receive and reply to messages. Per-agent bindings live on each agent's Channels tab.
    </p>
  </div>
  ...
  <Card>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
      {#each globalTypes as t (t.type)}
        <div class="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 flex items-center gap-3">
  ...
  <Card>
    ...
    {#if myChannels.length === 0}
      <p class="text-sm text-zinc-500">No channels connected. Open an agent's Channels tab to connect one.</p>
    {:else}
      <ul class="divide-y divide-zinc-800">
        {#each myChannels as c (c.type + ':' + c.account_id)}
          <li class="py-2 flex items-center gap-3">
            ...
            <span class="text-xs text-zinc-500 ml-auto">{c.enabled ? 'enabled' : 'disabled'}</span>
          </li>
        {/each}
      </ul>
    {/if}
