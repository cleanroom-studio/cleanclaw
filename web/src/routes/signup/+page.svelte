    ...
    {#if !open}
      <Card>
        ...
        <p class="text-sm text-zinc-400">
          Ask an administrator to enable public signups, or use an invite link
          if one was shared with you.
        </p>
      </Card>
    {:else}
      <Card>
        ...
        <form onsubmit={submit} class="space-y-3">
          <div>
            <Label for="su-u">Username</Label><Input
              id="su-u"
              bind:value={username}
              required
            />
          ...
          <div>
            <Label for="su-e">Email</Label><Input
              id="su-e"
              type="email"
              bind:value={email}
              required
          ...
          <div>
            <Label for="su-p">Password (min 8)</Label><Input
              id="su-p"
              type="password"
              bind:value={password}
              minlength={8}
          ...
          <div>
            <Label for="su-c">Confirm</Label><Input
              id="su-c"
              type="password"
              bind:value={confirm}
              minlength={8}
          ...
          <Button type="submit" class="w-full" disabled={submitting}
            >{submitting ? "Creating…" : "Sign up"}</Button
          >
        </form>
      </Card>
    {/if}
