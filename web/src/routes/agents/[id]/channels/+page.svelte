...
//
  ...
  import { listChannels, connectChannel, disconnectChannel, type ChannelInfo } from '$lib/api';
  ...
  const types = [
    { value: 'telegram', label: 'Telegram', help: 'Token from @BotFather. Webhook URL set automatically.' },
    { value: 'discord', label: 'Discord', help: 'Bot token from Discord developer portal.' },
    { value: 'slack', label: 'Slack', help: 'Bot user OAuth token (xoxb-…). App token (xapp-…) configured in /channels-config.' },
    { value: 'feishu', label: 'Feishu', help: 'App ID + App Secret from Feishu open platform.' },
    { value: 'line', label: 'LINE', help: 'Channel access token + channel secret from LINE console.' },
    { value: 'wechat', label: 'WeChat', help: 'Scan QR with WeChat to link the official account.' },
  ];
  ...
  async function connect(e: Event) {
    ...
    error = '';
    try {
      const r = await connectChannel(params.id, formType, formAccount, formToken);
      if (!r.ok) { error = r.error || 'connect failed'; return; }
      formAccount = '';
      formToken = '';
      await refresh();
    } catch (e) {
...
<div class="p-6 max-w-4xl mx-auto space-y-4">
  <div>
    <h2 class="text-2xl font-semibold tracking-tight">Channels · {params.id}</h2>
          ...
          {#each channels as c (c.type + ':' + c.account_id)}
            <tr class="border-b border-zinc-800/50">
              ...
              <td class="py-2 text-right">
                <button class="text-xs text-red-400 hover:underline" onclick={() => disconnect(c.type, c.account_id)}>Disconnect</button>
              </td>
            </tr>
          {/each}
        </tbody>
    ...
    <form onsubmit={connect} class="space-y-3">
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <select id="ch-t" bind:value={formType}
        ...
        <div>
          ...
          <Input id="ch-a" bind:value={formAccount} placeholder="bot1" required />
        </div>
      </div>
      <div>
        <label for="ch-tok" class="text-xs text-zinc-400">Bot token / secret</label>
        <Input id="ch-tok" type="password" bind:value={formToken} placeholder="••••••" required />
      </div>
      <p class="text-xs text-zinc-500">{types.find((t) => t.value === formType)?.help}</p>
