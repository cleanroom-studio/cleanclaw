  ...
  async function save() {
    ...
    try {
      await updateAgent(params.id, {
        soul, identity, system_prompt: systemPrompt,
        temperature, max_tokens: maxTokens, max_tool_iterations: maxIter,
        thinking,
      } as any);
      msg = 'Saved.';
    } catch (e) {
...
<div class="p-6 max-w-3xl mx-auto space-y-4">
  <div>
    <h2 class="text-2xl font-semibold tracking-tight">Customize · {agent?.name || params.id}</h2>
    <p class="text-sm text-zinc-400 mt-1">System prompt, persona, and runtime parameters.</p>
  </div>
  ...
  <Card>
    ...
    ></textarea>
    <p class="mt-2 text-xs text-zinc-500">Markdown supported. This is the primary persona; it takes precedence when set.</p>
  </Card>
  ...
  <Card>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div>
        ...
        <input id="cu-t" type="number" min="0" max="2" step="0.05" bind:value={temperature}
      ...
      <div>
        ...
        <input id="cu-mt" type="number" min="64" max="200000" bind:value={maxTokens}
          class="flex h-9 w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 text-sm" />
      </div>
      <div>
        <label for="cu-mi" class="text-xs text-zinc-400">Max tool iterations</label>
        <input id="cu-mi" type="number" min="1" max="32" bind:value={maxIter}
          class="flex h-9 w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 text-sm" />
      </div>
      <div>
        <label for="cu-th" class="text-xs text-zinc-400">Thinking mode</label>
        <input id="cu-th" bind:value={thinking} placeholder="auto / enabled / disabled"
          class="flex h-9 w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 text-sm" />
      </div>
    </div>
  </Card>
