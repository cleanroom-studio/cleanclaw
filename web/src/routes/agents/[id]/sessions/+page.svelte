<script lang="ts">
  ...
  import { listChatSessions, renameChatSession, deleteChatSession, type SessionInfo } from '$lib/api';
  ...
  async function refresh() {
    ...
    try {
      ...
      sessions = (r.sessions ?? []).slice().sort(
...
<div class="p-6 max-w-4xl mx-auto space-y-4">
  <div class="flex items-center gap-3">
    ...
    <Button size="sm" variant="outline" class="ml-auto" onclick={refresh}>Refresh</Button>
  </div>
  ...
  <Card>
    ...
    {:else if sessions.length === 0}
      <p class="text-sm text-zinc-500">No sessions yet. Start a chat to create one.</p>
    {:else}
          ...
          {#each sessions as s (s.key)}
            <tr class="border-b border-zinc-800/50">
              ...
              <td class="py-2 text-right space-x-2">
                {#if editing === s.key}
                  <button class="text-xs text-violet-300" onclick={saveTitle}>Save</button>
                  <button class="text-xs text-zinc-500" onclick={() => (editing = null)}>×</button>
                {:else}
                  <button class="text-xs text-violet-300 hover:underline" onclick={() => open(s.key)}>Open</button>
                  <button class="text-xs text-zinc-500 hover:underline" onclick={() => startEdit(s)}>Rename</button>
                  <button class="text-xs text-red-400 hover:underline" onclick={() => remove(s.key)}>Delete</button>
                {/if}
              </td>
            </tr>
          {/each}
