<script lang="ts">
  ...
  const WEB_SEARCH_PROVIDERS: ProviderMeta[] = [
    {
      key: "duckduckgo",
      label: "DuckDuckGo",
      description: "Free. No key. HTML scrape. Works globally — good default.",
    ...
    },
    {
      ...
      label: "Brave Search",
      description:
        "Independent index. Fast, high quality. 2000 free queries / month.",
    ...
    },
    {
      ...
      description: "Web Search API v7. Good for enterprise.",
    ...
    },
    {
      key: "google",
      label: "Google CSE",
      description: "Programmable Search Engine. Limited 100/day free.",
      docs: "https://developers.google.com/custom-search",
      needsKey: true,
      needsEndpoint: true,
      endpointHint: "cx=<engine-id>",
    },
    {
      key: "baidu",
      label: "Baidu 百度",
      description:
        "Free. No key. HTML scrape. Best from CN IPs; may captcha elsewhere.",
    ...
    },
    {
      key: "searxng",
      label: "SearXNG",
      description: "Meta-search. Self-hosted instance URL required.",
      docs: "https://searxng.org",
      needsKey: false,
      needsEndpoint: true,
      endpointHint: "https://searx.example.com",
    },
    {
      key: "exa",
      label: "Exa",
      description: "Neural search. 1000 free / month.",
      docs: "https://exa.ai",
      needsKey: true,
      needsEndpoint: false,
    },
    {
      key: "none",
      label: "Disabled (none)",
      description: "Disable web search entirely.",
      docs: "",
      needsKey: false,
      needsEndpoint: false,
    },
  ...
  const CATEGORIES: Cat[] = [
    ...
    },
    {
      key: "image_gen",
      label: "Image Gen",
      desc: "DALL·E / Fal / Replicate.",
      options: IMAGEGEN_PROVIDERS,
    },
    {
      key: "tts",
      label: "Text-to-Speech",
      desc: "OpenAI / ElevenLabs / Fish / minimax.",
      options: TTS_PROVIDERS,
    },
    {
      key: "webfetch",
      label: "Web Fetch",
      desc: "Direct / Firecrawl / Jina.",
      options: WEBFETCH_PROVIDERS,
    },
  ];

  function descriptions_web_search() {
    return "Search the web via DuckDuckGo / Brave / Bing / Google / Baidu / Exa / SearXNG.";
  }
  ...
  async function setProvider(category: Category, value: string) {
    if (!tools[category])
      tools[category] = { enabled: true, provider: value } as any;
  ...
  function toggle(category: Category, value: boolean) {
    if (!tools[category])
      tools[category] = { enabled: value, provider: "duckduckgo" } as any;
...
<div class="p-6 max-w-4xl mx-auto space-y-4">
  <div>
    ...
    <p class="text-sm text-zinc-400 mt-1">
      Per-category provider routing and on/off toggles. Search credentials are
      stored system-wide; the agent picks up the row on the next turn.
    </p>
  </div>
    ...
    {:else}
      <ul class="divide-y divide-zinc-800">
        {#each CATEGORIES as cat (cat.key)}
          {@const cfg = (tools[cat.key] as any) || {
            enabled: true,
            provider: "none",
          }}
          ...
          <li class="py-4 space-y-3">
            <div class="flex items-center gap-3">
              ...
              <Switch
                checked={enabled}
                onchange={() => toggle(cat.key, !enabled)}
              />
            </div>

            {#if cat.key === "web_search" && meta && provider !== "none"}
              <div
                class="ml-2 pl-3 border-l border-zinc-800 space-y-2 text-xs text-zinc-400"
              >
                ...
                {#if meta.docs}
                  <div>
                    Docs: <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      class="text-violet-300 hover:underline">{meta.docs}</a
                    >
                  </div>
                {/if}
                {#if meta.needsKey}
