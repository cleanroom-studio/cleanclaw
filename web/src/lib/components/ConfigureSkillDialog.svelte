<script lang="ts">
  ...
  export type SkillEntryView = { apiKey?: string; env?: Record<string, string> };
    ...
    if (open && skill) {
      void (async () => {
        try {
          ...
          apiKey = e.apiKey || '';
          envPairs = Object.entries(e.env || {}).map(([key, value]) => ({ key, value }));
        } catch {}
      })();
    }
  });
...
<Dialog bind:open title={skill ? `Configure ${skill.name}` : 'Configure skill'}>
  <div class="space-y-4 text-sm">
    <div class="space-y-1.5">
      ...
      <Input id="sk-key" type="password" bind:value={apiKey} placeholder="sk-…" />
    </div>
      ...
      <div class="space-y-2">
        {#each envPairs as p, i (i)}
          <div class="flex items-center gap-2">
            ...
            <button type="button" class="text-zinc-500 hover:text-red-400 text-xs" onclick={() => removeEnv(i)}>×</button>
          </div>
        {/each}
        <button type="button" class="text-xs text-violet-300" onclick={addEnv}>+ Add env var</button>
      </div>
    </div>
