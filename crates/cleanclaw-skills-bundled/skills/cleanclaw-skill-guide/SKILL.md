...
description: Create new skills for CleanClaw agents. Use when the user asks to create a skill, turn a workflow into a skill, or build reusable automation. Also use when discussing skill format, structure, or best practices.
---

# CleanClaw Skill Guide

Guide for creating new skills that CleanClaw agents can discover and use.
...
| `metadata.cleanclaw.emoji` | No | Display icon |
| `metadata.cleanclaw.always` | No | If true, full content is always in system prompt |
| `metadata.cleanclaw.os` | No | OS requirements (darwin, linux, windows) |
| `metadata.cleanclaw.requires.bins` | No | All listed binaries must exist on PATH |
| `metadata.cleanclaw.requires.anyBins` | No | At least one must exist |
| `metadata.cleanclaw.requires.env` | No | Required environment variables |
| `metadata.cleanclaw.primaryEnv` | No | Maps config apiKey to this env var |
...
4. **Save** — Write to `{agentDir}/skills/{skill-name}/SKILL.md` for agent-level, or `~/.cleanclaw/skills/{skill-name}/SKILL.md` for user-level
