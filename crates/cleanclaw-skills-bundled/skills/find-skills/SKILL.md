...
Install with `-g -y` — the CleanClaw sandbox bind-mounts the global install location to the chatter's host skill bucket, so a `-g` install lands in `~/.cleanclaw/users/<uid>/skills/<name>/` on host and is visible to the next chat turn.
